...
#define Instruction InstructionPEG
...
static const char *_match (lua_State *L,
  ...
  for (;;) {
    ...
    switch ((Opcode)p->i.code) {
      ...
      case ICloseRunTime: {
        ...
        lua_Integer res = lua_tointeger(L, fr);  /* offset */
        int n = lua_gettop(L) - fr;  /* number of new captures */
        if (res == 0) {  /* may not be a number */
      ...
      case IOpenCall: {
        lua_rawgeti(L, penvidx(ptop), p->i.offset - 1);
  ...
  while (p != e) {
    switch ((Opcode)p->i.code) {
      ...
      case IOpenCall: {
        ...
        back[backtop].s = NULL;
        back[backtop++].p = p + 1;
        p = op + getposition(L, postable, p->i.offset - 1);
        continue;
      }
      case IBackCommit:
      case ICommit: {
...
** '0' to the value at stack position 'vidx'.
*/
...
static Instruction *fix_l (lua_State *L, int t) {
  ...
  lua_pushinteger(L, 0);  /* default initial rule */
  /* collect patterns and compute sizes */
  lua_pushnil(L);
  while (lua_next(L, t) != 0) {
    int l;
    if (lua_tonumber(L, -2) == 0 && lua_isstring(L, -1)) {
  ...
  for (i = 0; i < totalsize; i += sizei(p + i)) {
    if (p[i].i.code == IOpenCall) {
      int pos = getposition(L, base + 1, p[i].i.offset - 1);
...
static Instruction *getpatt (lua_State *L, int idx, int *size) {
  Instruction *p = checkpattern(L, idx);
...
#define getfromenv(cs,v)	lua_rawgeti((cs)->L, penvidx((cs)->ptop), (v) - 1)
...
static int runtimecap (lua_State *L, Capture *close, Capture *ocap,
  ...
  lua_pushinteger(L, s - o);  /* current position */
  n = pushallvalues(&cs, 0);
...
static int pushcapture (CapState *cs) {
  ...
  switch (captype(cs->cap)) {
    case Cposition: {
      lua_pushinteger(cs->L, cs->cap->s - cs->s);
      cs->cap++;
      return 1;
    }
    case Cconst: {
...
static int getcaptures (lua_State *L, const char *s, const char *r, int ptop) {
  ...
  if (n == 0) {  /* no capture values? */
    lua_pushinteger(L, r - s);  /* return only end position */
    n = 1;
  }
  return n;
}
...
static int any_l (lua_State *L) {
  lua_Integer n = luaL_optinteger(L, 1, 1);
  luaL_argcheck(L, n >= 0, 1, "negative value");
  any(L, n, 0, NULL);
  return 1;
}


static int choice_l (lua_State *L) {
  (void)checkpattern(L, 1);
  int n = lua_gettop(L);
  if (n > 1) {
    int i;
    union_l(L);
...
static int empty_l (lua_State *L) {
  newpatt(L, 0);  /* empty pattern (always succeeds) */
  return 1;
}


static int except_l (lua_State *L) {
  (void)checkpattern(L, 1);
  int n = lua_gettop(L);
  if (n > 1) {
    int i;
...
static int eos_l (lua_State *L) {
  Instruction *p = newpatt(L, 2);
  setinstaux(p, IAny, 2, 1);
  setinst(p + 1, IFail, 0);
  return 1;
}


static int fail_l (lua_State *L) {
  Instruction *p = newpatt(L, 1);
  setinst(p, IFail, 0);
  return 1;
}
...
static int grammar_l (lua_State *L) {
  luaL_checktype(L, 1, LUA_TTABLE);
  fix_l(L, 1);
  return 1;
}
...
static int followed_by_l (lua_State *L) {
  Instruction *p1 = checkpattern(L, 1);
  int l1 = pattsize(L, 1);
  CharsetTag st1;
...
static int many_l (lua_State *L) {
  Instruction *p1 = checkpattern(L, 1);
  int l1 = pattsize(L, 1);
  CharsetTag st;
  Instruction *op;
  if (tocharset(p1, &st) == ISCHARSET)
    return repeatcharset(L, st.cs, l1, 0);
  if (isheadfail(p1))
    op = repeatheadfail(L, l1, 0);
  else
    op = repeats(L, p1, l1, 0);
  optimizecaptures(op);
  optimizejumps(op);
  return 1;
}
...
static int optional_l (lua_State *L) {
  Instruction *p1 = checkpattern(L, 1);
  int l1 = pattsize(L, 1);
  if (isheadfail(p1))
    optionalheadfail(L, l1, 1);
  else
    optionals(L, l1, 1);
  return 1;
}
...
static int sequence_l (lua_State *L) {
  (void)checkpattern(L, 1);
  int n = lua_gettop(L);
  if (n > 1) {
    int i;
...
static int some_l (lua_State *L) {
  Instruction *p1 = checkpattern(L, 1);
  int l1 = pattsize(L, 1);
  CharsetTag st;
  Instruction *op;
  if (tocharset(p1, &st) == ISCHARSET)
    return repeatcharset(L, st.cs, l1, 1);
  if (isheadfail(p1))
    op = repeatheadfail(L, l1, 1);
  else
    op = repeats(L, p1, l1, 1);
  optimizecaptures(op);
  optimizejumps(op);
  return 1;
}
...
static struct luaL_Reg pattreg[] = {
  ...
  {"group", group_l},
  {"literal", literal_l},
  {"followed_by", followed_by_l},
  {"many", many_l},
  {"matchtime", matchtime_l},
  {"optional", optional_l},
  {"position", position_l},
  {"range", range_l},
  {"replace", rcapture_l},
