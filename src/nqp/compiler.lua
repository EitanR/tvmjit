...
(!do
        ...
        (!let _new (!lambda (meta a)
                ...
                (!let class (!call1 Mu_BUILD p6class ("name": name "mt": mt "_new": (!and new (!lambda () (!return new))))))
                (!assign (!index mt "__index") (!index (!index class "_VALUES") "proto"))
                (!return (!call setmetatable class (!index (!index meta "_VALUES") "mt")))))
        (!let proto ("add_method": _add_method))
        (!assign p6class ("_VALUES": (
                "name": "NQP::Metamodel::ClassHOW"
                "proto": proto
                "attributes": ()
                "methods": ("add_method": _add_method)
                "parents": ("n": 0)
                "roles": ("n": 0)
                "ISA": (0: p6class "n": 1)
                "_new": _new
                "mt": ("__tostring": (!lambda (o) (!return (!callmeth o str)))) )))
        (!assign (!index p6class "_CLASS") p6class)
        (!assign (!index (!index (!index p6class "_VALUES") "mt") "__index") proto)
...
(!call (!index p6class "add_attribute") (p6class ("name": "_new" "default": (!lambda () (!return (!lambda (class a)
...
(!call (!index p6class "add_method") (p6class "new" (!lambda (meta a)
                (!return (!call (!index (!index meta "_VALUES") "_new") meta a)))))
...
(!let p6role (!callmeth1 p6class new ("NQP::Metamodel::RoleHOW" (!lambda (meta a)
                (!define name (!call unpack a))
                (!assign name (!or (!and (!eq (!call1 type name) "table") (!index name "_VALUES")) name))
                (!call checktype "new" 2 name "string")
                (!return (!call setmetatable (!call1 Mu_BUILD meta ("name": name)) (!index (!index meta "_VALUES") "mt")))))))
(!do
(!assign (!index _P6PKG "NQP::Metamodel::RoleHOW") p6role)
(!assign (!index (!index p6role "_VALUES") "mt") (
        "__index": (!index (!index p6role "_VALUES") "proto")
        "__tostring": (!lambda (o) (!return (!callmeth o str))) ))
...
(!let p6module (!callmeth1 p6class new ("NQP::Metamodel::ModuleHOW" (!lambda (meta a)
                (!define name (!call unpack a))
                (!assign name (!or (!and (!eq (!call1 type name) "table") (!index name "_VALUES")) name))
                (!call checktype "new" 2 name "string")
                (!return (!call setmetatable (!call1 Mu_BUILD meta ("name": name)) (!index (!index meta "_VALUES") "mt")))))))
...
(!assign (!index (!index p6module "_VALUES") "mt") (
        "__index": (!lambda (t k) (!return (!or (!call1 rawget (!index t "_VALUES") k) (!index (!index (!index (!index t "_CLASS") "_VALUES") "proto") k))))
        "__newindex": (!lambda (t k v) (!call rawset (!index t "_VALUES") k v))
        "__tostring": (!lambda (o) (!return (!callmeth o str))) ))
...
(!let p6array (!callmeth1 p6class new ("Array" (!lambda (class a)
                (!define array (!or (!call unpack a) ("n": 0)))
...
(!assign (!index (!index p6array "_VALUES") "mt") (
        "__index": (!lambda (t k)
                        ...
                        (!if (!eq (!call1 type k) "number")
                             (!do (!if (!lt k 0)
                                       (!assign k (!add (!index (!index t "_VALUES") "n") k)))
                                  (!return (!call rawget (!index t "_VALUES") k)))
                             (!return (!index (!index (!index (!index t "_CLASS") "_VALUES") "proto") k))))
        "__newindex": (!lambda (t k v)
                        (!assign k (!or (!and (!eq (!call1 type k) "table") (!index k "_VALUES")) k))
                        (!if (!ge k (!index (!index t "_VALUES") "n"))
                             (!assign (!index (!index t "_VALUES") "n") (!add k 1)))
                        (!call rawset (!index t "_VALUES") k v))
        "__tostring": (!lambda (t) (!return (!callmeth t str))) ))
...
(!call (!index p6class "add_method") (p6array "pop" (!lambda (self)
                ...
                (!if (!lt n 0)
                     (!call error "can't pop from empty array"))
                (!let v (!index self n))
...
(!call (!index p6class "add_method") (p6array "shift" (!lambda (self)
                (!let n (!sub (!index (!index self "_VALUES") "n") 1))
                (!if (!lt n 0)
                     (!call error "can't shift from empty array"))
                (!let v (!index self 0))
                (!loop i 0 n 1
...
(!call (!index p6class "add_method") (p6array "_itern" (!lambda (self a)
                ...
                (!return (!lambda ()
                                ...
                                (!let t ())
                                (!loop _ 1 n 1
                                        (!assign (!index t (!add (!len t) 1)) (!index (!index self "_VALUES") i))
                                        (!assign i (!add i 1)))
...
(!let p6hash (!callmeth1 p6class new ("Hash" (!lambda (class a)
                (!define t (!or (!call1 unpack a) ()))
                (!assign t (!or (!index t "_VALUES") t))
                (!let hash ())
...
(!assign (!index (!index p6hash "_VALUES") "mt") (
        "__index": (!lambda (t k) (!return (!or (!call1 rawget (!index t "_VALUES") (!call1 keystr k)) (!index (!index (!index (!index t "_CLASS") "_VALUES") "proto") k))))
        "__newindex": (!lambda (t k v) (!call rawset (!index t "_VALUES") (!call1 keystr k) v))
        "__tostring": (!lambda (t) (!return (!callmeth t str))) ))
...
(!call (!index p6class "add_method") (p6hash "bool" (!lambda (self)
                (!return (!ne (!call1 next (!index self "_VALUES")) !nil)))))
...
(!call (!index p6class "add_method") (p6hash "values" (!lambda (self)
                (!let t ())
                (!define n 0)
                (!for (_ v) ((!call pairs (!index self "_VALUES")))
                        (!assign (!index t n) v)
                        (!assign n (!add n 1)))
                (!assign (!index t "n") n)
                (!return (!callmeth (!index _P6PKG "Array") new (t))))))
...
(!call (!index p6class "add_parent") (p6array p6any))
...
(!let p6str (!callmeth1 p6class new ("str" (!lambda (self a)
...
(!assign (!index (!index p6str "_VALUES") "mt") ("__index": (!index (!index p6str "_VALUES") "proto")))
...
(!call (!index p6class "add_method") (p6str "num" (!lambda (self)
                (!return (!call assert (!call1 tonumber self) "numify failed")))))
...
(!let p6bool (!callmeth1 p6class new ("bool" (!lambda (self a)
...
(!let p6nil (!callmeth1 p6class new ("nil" (!lambda ()
                (!return !nil)))))
(!do
...
(!let p6num (!callmeth1 p6class new ("num" (!lambda (self a)
                (!define num (!call unpack a))
                (!call checktype "new" 1 num "number")
                (!return num)))))
(!do
(!let floor (!index math "floor"))
(!assign (!index (!index p6num "_VALUES") "mt") ("__index": (!index (!index p6num "_VALUES") "proto")))
...
(!let p6str (!callmeth1 p6class new ("Str" (!lambda (class a)
                (!define str (!call unpack a))
...
(!assign (!index (!index p6str "_VALUES") "mt") (
        ...
        "__tostring": (!lambda (o) (!return (!index o "_VALUES"))) ))
(!call (!index p6class "add_method") (p6str "WHAT" (!lambda (self)
                (!return p6str))))
(!call (!index p6class "add_method") (p6str "Str" (!lambda (self)
                (!return self))))
...
(!let p6bool (!callmeth1 p6class new ("Bool" (!lambda (class a)
                (!define bool (!call unpack a))
...
(!let p6nil (!callmeth1 p6class new ("Nil" (!lambda (class)
...
(!let p6num (!callmeth1 p6class new ("Num" (!lambda (class a)
                (!define num (!call unpack a))
